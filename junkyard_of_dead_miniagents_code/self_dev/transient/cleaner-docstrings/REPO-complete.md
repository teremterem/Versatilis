...
class PromiseResolvedEventHandler(Protocol):
    """
    A protocol for Promise resolution event handlers. A promise resolution event is a function that is scheduled
    to be called after Promise.aresolve() finishes resolving the promise. "Scheduled" means that the function is
    passed to the async event loop for execution without blocking the current coroutine.
    """
...
class PromisingContext:
    ...
    ensure that all the async tasks finish before this context manager is exited).
    """
